...
int CaseLinear(const double& b, const double& c) {
    if (b != 0)
        return 1;
    else if (c != 0)
        return -1;
...
int CaseQuadratic(const double& a, const double& b, const double& c) {
    if (D(a, b, c) > 0)
        return 2;
    else if (D(a, b, c) == 0)
        return 1;
...
int GetDistinctRealRootCount(double a, double b, double c) {
    if (a == 0)
        return CaseLinear(b, c);
    else
        return CaseQuadratic(a, b, c);
}
